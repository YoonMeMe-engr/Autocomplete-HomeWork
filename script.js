...
autoCompleteTag.addEventListener("keyup" , (event) => {
  ...
  filterProducts = products.filter((produce) => {
    return produce.title.toLowerCase().includes(searchText);
  });
  ...
  if (hasProductToShow) {
    for ( let i = 0; i < filterProducts.length; i ++ ) {
      ...
      productItemContainer.classList.add("productItem");
      ...
      productPrice.append(filterProducts[i].price);

      const productImage = document.createElement("img");
      productImage.classList.add("productImage");
      productImage.src = filterProducts[i].image;

      productItemContainer.append(productName , productPrice , productImage);
...
const navigateAndSelectProduct = (key) => {
  if (key === "ArrowDown") {
    if (indexToShow === filterProducts.length -1) {
      indexToShow = -1;
      deselectProducts();
      return;
    };
...
const selectProducts = (index) => {
  const selectProduct = filterProducts[indexToShow].id.toString();
  const productItemContainerToSelect = document.getElementById(selectProduct);
...
const deselectProducts = () => {
  const deselectProduct = document.getElementsByClassName("selected")[0];
  deselectProduct.style.backgroundColor = "white";
  deselectProduct.firstChild.style.color = "black";
  deselectProduct.classList.remove("selected");
};
